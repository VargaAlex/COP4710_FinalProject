...
<?php
	...
	if($is_admin or $is_superadmin) {
		...
		if ($_SERVER["REQUEST_METHOD"] == "POST") {
			...
			$event_rso = mysqli_real_escape_string($db,$_POST['rsoSelect']);
					...
					if($res = mysqli_query($db,$sql)) {
						...
						if($res = mysqli_query($db,$sql)) {
							if($row = mysqli_fetch_array($res)) {
								...
								$sql = "insert into rso_events(e_id, rso_id) values($e_id, $event_rso) ";
								if($res = mysqli_query($db,$sql)) {
									header("Location:event_details.php?e_id=$e_id");
									die();
								}
								else echo "Failed to insert new rso event.";
							}
						}
...
<p>
Sponsor: 
<select name="rsoSelect">
<option value="">Select...</option>

<?php
	$sql = "select rso_id, name from rso where a_id = $is_admin";
	if($res = mysqli_query($db,$sql)) {
		while($row = mysqli_fetch_array($res)) {
			$rso_id = $row[0];
			$rso_name = $row[1];
			echo "
			<option value=$rso_id>$rso_name</option>
			";
		}
	}
?>
